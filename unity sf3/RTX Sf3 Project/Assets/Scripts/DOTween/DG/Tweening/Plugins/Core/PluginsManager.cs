...
namespace DG.Tweening.Plugins.Core
{
	internal static class PluginsManager
	{
		...
		internal static ABSTweenPlugin<T1, T2, TPlugOptions> GetDefaultPlugin<T1, T2, TPlugOptions>() where TPlugOptions : struct
		{
			...
			if (typeFromHandle == typeof(Vector3Wrapper) && typeFromHandle == typeFromHandle2)
			{
				if (_vector3Plugin == null)
				{
					_vector3Plugin = new Vector3WrapperPlugin();
				}
			...
			else if (typeFromHandle == typeof(QuaternionWrapper))
			{
				...
				else
				{
					...
					{
						_quaternionPlugin = new QuaternionWrapperPlugin();
					}
					tweenPlugin = _quaternionPlugin;
				}
			}
			else if (typeFromHandle == typeof(Vector2Wrapper))
			{
				if (_vector2Plugin == null)
				{
					_vector2Plugin = new Vector2WrapperPlugin();
				}
			...
			else if (typeFromHandle == typeof(ColorWrapper))
			{
				if (_colorPlugin == null)
				{
					_colorPlugin = new ColorWrapperPlugin();
				}
			...
			else if (typeFromHandle == typeof(Vector4Wrapper))
			{
				if (_vector4Plugin == null)
				{
					_vector4Plugin = new Vector4WrapperPlugin();
				}
