...
namespace DG.Tweening
{
	public static class ShortcutExtensions46
	{
		...
		{
			return DOTween.To(() => new Vector2(target.flexibleWidth, target.flexibleHeight), delegate(Vector2Wrapper x)
			{
		...
		{
			return DOTween.To(() => new Vector2(target.minWidth, target.minHeight), delegate(Vector2Wrapper x)
			{
		...
		{
			return DOTween.To(() => new Vector2(target.preferredWidth, target.preferredHeight), delegate(Vector2Wrapper x)
			{
		...
		{
			return DOTween.To(() => target.effectColor, delegate(ColorWrapper x)
			{
		...
		{
			return DOTween.ToAlpha(() => target.effectColor, delegate(ColorWrapper x)
			{
				target.effectColor = x;
			}, endValue, duration).SetTarget(target);
		}
		...
		{
			return DOTween.To(() => target.effectDistance, delegate(Vector2Wrapper x)
			{
		...
		{
			return DOTween.To(() => target.anchoredPosition, delegate(Vector2Wrapper x)
			{
		...
		{
			return DOTween.To(() => target.anchoredPosition3D, delegate(Vector3Wrapper x)
			{
		...
		{
			return DOTween.To(() => target.anchorMax, delegate(Vector2Wrapper x)
			{
				target.anchorMax = x;
			}, endValue, duration).SetOptions(snapping).SetTarget(target);
		}
		...
		{
			return DOTween.To(() => target.anchorMin, delegate(Vector2Wrapper x)
			{
				target.anchorMin = x;
			}, endValue, duration).SetOptions(snapping).SetTarget(target);
		}
		...
		{
			return DOTween.To(() => target.pivot, delegate(Vector2Wrapper x)
			{
				target.pivot = x;
			}, endValue, duration).SetTarget(target);
		}
		...
		{
			return DOTween.To(() => target.sizeDelta, delegate(Vector2Wrapper x)
			{
				target.sizeDelta = x;
			}, endValue, duration).SetOptions(snapping).SetTarget(target);
		}
		...
		public static Sequence DOJumpAnchorPos(this RectTransform target, Vector2 endValue, float jumpPower, int numJumps, float duration, bool snapping = false)
		{
			...
			Sequence s = DOTween.Sequence().Append(DOTween.To(() => target.anchoredPosition, delegate(Vector2Wrapper x)
			{
				target.anchoredPosition = x;
			}, new Vector2(endValue.x, 0f), duration).SetOptions(AxisConstraint.X, snapping).SetEase(Ease.Linear)).Join(DOTween.To(() => target.anchoredPosition, delegate(Vector2Wrapper x)
			{
				target.anchoredPosition = x;
		...
		{
			return DOTween.To(() => new Vector2Wrapper(target.horizontalNormalizedPosition, target.verticalNormalizedPosition), delegate(Vector2Wrapper x)
			{
