...
namespace Google.Protobuf
{
	public sealed class JsonParser
	{
		public sealed class Settings
		{
			...
			[CompilerGenerated]
			private readonly int _003CRecursionLimit_003Ek__BackingField;
			...
			public int RecursionLimit
			{
				...
				get
				{
					return _003CRecursionLimit_003Ek__BackingField;
				}
			}
			...
			static Settings()
			{
				_003CDefault_003Ek__BackingField = new Settings(64);
			}
			...
			{
				_003CRecursionLimit_003Ek__BackingField = recursionLimit;
		...
		private void Merge(IMessage message, JsonTokenizer tokenizer)
		{
			...
			Action<JsonParser, IMessage, JsonTokenizer> value;
			if (message.Descriptor.IsWellKnownType && WellKnownTypeHandlers.TryGetValue(message.Descriptor.FullName, out value))
			{
			...
			while (true)
			{
				...
				FieldDescriptor value2;
				if (!dictionary.TryGetValue(stringValue, out value2))
				{
					break;
				}
				if (value2.ContainingOneof != null)
				{
					...
					{
						throw new InvalidProtocolBufferException(string.Format("Multiple values specified for oneof {0}", value2.ContainingOneof.Name));
					}
				}
