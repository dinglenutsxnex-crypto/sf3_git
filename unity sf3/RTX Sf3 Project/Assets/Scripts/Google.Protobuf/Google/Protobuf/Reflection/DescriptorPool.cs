...
namespace Google.Protobuf.Reflection
{
	internal sealed class DescriptorPool
	{
		...
		{
			IDescriptor value;
			descriptorsByName.TryGetValue(fullName, out value);
			T val = value as T;
			if (val != null)
			{
				return val;
		...
		internal void AddPackage(string fullName, FileDescriptor file)
		{
			...
			IDescriptor value;
			if (descriptorsByName.TryGetValue(fullName, out value) && !(value is PackageDescriptor))
			{
		...
		internal void AddSymbol(IDescriptor descriptor)
		{
			...
			IDescriptor value;
			if (descriptorsByName.TryGetValue(fullName, out value))
			{
				int num = fullName.LastIndexOf('.');
		...
		internal FieldDescriptor FindFieldByNumber(MessageDescriptor messageDescriptor, int number)
		{
			...
			fieldsByNumber.TryGetValue(new DescriptorIntPair(messageDescriptor, number), out value);
			return value;
		}
		...
		internal EnumValueDescriptor FindEnumValueByNumber(EnumDescriptor enumDescriptor, int number)
		{
			...
			enumValuesByNumber.TryGetValue(new DescriptorIntPair(enumDescriptor, number), out value);
			return value;
		}
		...
		internal void AddFieldByNumber(FieldDescriptor field)
		{
			...
			FieldDescriptor value;
			if (fieldsByNumber.TryGetValue(key, out value))
			{
