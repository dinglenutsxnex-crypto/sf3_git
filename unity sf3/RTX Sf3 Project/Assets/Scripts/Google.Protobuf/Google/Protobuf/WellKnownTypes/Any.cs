...
namespace Google.Protobuf.WellKnownTypes
{
	public sealed class Any : IMessage<Any>, IMessage, IEquatable<Any>, IDeepCloneable<Any>
	{
		...
		public T Unpack<T>() where T : IMessage, new()
		{
			...
			{
				throw new InvalidProtocolBufferException(string.Format("Full type name for {0} is {1}; Any message's type url is {2}", val.Descriptor.Name, val.Descriptor.FullName, TypeUrl));
			}
			val.MergeFrom(Value);
			return val;
		}
