	...
	public class SFSArray : ISFSArray, ICollection, IEnumerable
	{
		...
		private string Dump()
		{
			...
			for (int i = 0; i < dataHolder.Count; i++)
			{
				...
				string value = ((type != 18) ? ((type != 17) ? ((type != 0) ? ((type <= 8 || type >= 19) ? sFSDataWrapper.Data.ToString() : string.Concat("[", sFSDataWrapper.Data, "]")) : "NULL") : (sFSDataWrapper.Data as SFSArray).GetDump(false)) : (sFSDataWrapper.Data as SFSObject).GetDump(false));
