	...
	public class ConfigLoader : IDispatchable
	{
		...
		private void TryParse()
		{
			...
			try
			{
				...
				if (GetNodeText(xMLNode, "ip") == null)
				{
					smartFox.Log.Error("Required config node missing: ip");
				}
				if (GetNodeText(xMLNode, "port") == null)
				{
					smartFox.Log.Error("Required config node missing: port");
				}
				if (GetNodeText(xMLNode, "udpIp") == null)
				{
					smartFox.Log.Error("Required config node missing: udpIp");
				}
				...
				configData.Host = GetNodeText(xMLNode, "ip");
				configData.Port = Convert.ToInt32(GetNodeText(xMLNode, "port"));
				configData.UdpHost = GetNodeText(xMLNode, "udpIp");
